...
function ENT:CreateBurstSpotlight( brightness, fov, distance, duration )
    ...
    -- Auto cleanup if bot is removed
...
function ENT:PerformBurst( burstNum )
    ...
    for _, ent in ipairs( nearbyEnts ) do
        ...
        if angleToEnt > self.BreachConeAngle / 2 then continue end
        ...
        if tr.Entity ~= ent and tr.Fraction <= 0.9 then continue end
...
ENT.MyClassTask = {
    ...
    BehaveUpdatePriority = function( self, data )
        ...
        if dist > self.BreachRange then return end
        if dist < 150 then return end
        if not self:CanTakeAction( "ContainmentBreach" ) then return end
        
        self:TakeAction( "ContainmentBreach" )
    end,
    
    BehaveUpdateMotion = function( self, data )
        if not self.BreachActive then return end
        
        self.loco:SetVelocity( Vector( 0, 0, 0 ) )
        
        if not self.BreachLockedAngles then return end
